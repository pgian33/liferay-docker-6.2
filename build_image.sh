...
function date {
	...
	then
		...
		if [ "$(uname)" == "Darwin" ]
		then
			echo $(/bin/date -jf "%a %b %e %H:%M:%S %Z %Y" "${1}" "${2}" +"%Y%m%d%H%M")
		elif [ -e /bin/date ]
		then
			echo $(/bin/date -d "${1}" "${2}" +"%Y%m%d%H%M")
		else
			echo $(/usr/bin/date -d "${1}" "${2}" +"%Y%m%d%H%M")
		fi
	fi
}
...
function main {
	check_utils 7z curl docker unzip
	...
	cp -r template/home/.bashrc ${timestamp}
